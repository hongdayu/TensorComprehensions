...
isl::multi_union_pw_aff extractDomainToThread(
  ...
  for (auto mapping : tree->collect(tree, ScheduleTreeType::MappingFilter)) {
    auto mappingNode = mapping->elemAsBase<ScheduleTreeElemMappingFilter>();
 ...
 * Construct a mapping
 *
 *  block[x] -> warp[floor((x)/s_w)]
 *  block[x, y] -> warp[floor((x + s_x * (y))/s_w)]
 *  block[x, y, z] -> warp[floor((x + s_x * (y + s_y * (z)))/s_w)]
 *
...
isl::multi_aff constructThreadToWarp(
  ...
  auto domain = space.named_set_from_params_id(id, block.view.size());
  auto warp = space.named_set_from_params_id(isl::id(ctx, kWarp), 1);
  auto aff = isl::aff::zero_on_domain(domain);

  auto nThread = block.view.size();
  auto identity = isl::multi_aff::identity(domain.map_from_set());
  ...
  warp = domain.product(warp).unwrap();
  return isl::multi_aff(warp, isl::aff_list(aff));
}
} // namespace
